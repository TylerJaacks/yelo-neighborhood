...
// Yelo.Debug not compatible with Complex_4627Debug.bin bios
...
namespace Yelo.Debug
{
	...
	public partial class Xbox : IDisposable
	{
            ...
            if (connected)
            {
                ...
                while (line[0] != '.')
                {
                    ...
                    if (module.Name == "xbdm.dll" && module.BaseAddress != 0xB0000000)
                        throw new Exception("You seem to be most likely running the Complex v4627 Debug Bios.  Yelo.Debug is not compatible with this bios.");
		...
		public void RegisterNotificationSession(int port)
		{
			try
			{
                if (notificationSessionEnabled)
                {
                    ...
                    //StatusResponse res = SendCommand("notify");
                    //if (res.Type == ResponseType.NowNotifySession && notificationListener.Pending())
                    SendCommand("notifyat {0} drop", port);
                    StatusResponse res = SendCommand("notifyat port={0}", port);
        ...
        { LaunchTitle(path, true); }

        public void LaunchTitle(string path, bool reconnect)
        {
            FlushSocketBuffer();
            connection.Client.Send(ASCIIEncoding.ASCII.GetBytes("magicboot title=\"" + path + "\" debug" + Environment.NewLine));
            if (connection.Available > 0)
                connection.Client.Receive(new byte[connection.Available]);
            if(reconnect) Reconnect(15000);
        }
		#endregion
